...
**iOS 링크는 릴리스가 아니라 Pages 사이트의 파일을 가리킨다.** 안드로이드는 릴리스 자산
그대로다. 비대칭이지만 의도한 것이다 — APK 는 카톡이 아니라 시스템이 설치한다.

이유는 헤더 하나다. **릴리스 자산은 `Content-Disposition: attachment` 를 달고 나온다.**
그게 붙으면 브라우저는 내려받는 것 말고 아무것도 못 한다. 파일 앱을 뒤져 공유 시트로
카톡에 넘기는 긴 구간이 거기서 시작됐다. **Pages 에는 그 헤더가 없어서 카카오톡이 파일을
그대로 가로채 바로 설치한다** — `테마 다운로드 완료` 가 뜨고 끝난다. 채팅방도, 파일 선택도,
적용하기도 없다. 카카오 공식 안내가 말하는 "URL 을 열면 〈카카오톡으로 열기〉" 가 이것이다.

**그래서 iOS 링크에 릴리스 자산을 쓰면 안 된다.** 같은 파일이어도 헤더가 길을 막는다.
이게 이 저장소에서 "받는 사람은 릴리스 자산에서 받는다" 규칙의 유일한 예외다.

**카카오톡 안에서 README 를 열면 제일 확실하다.** 인앱 브라우저에서 확인했다.
다른 브라우저에서는 그냥 내려받아질 수 있고, 그때를 위해 `docs/share.html` 이 있다 —
버튼 하나가 파일을 iOS 공유 시트에 올린다. 두 번째 길이지 기본 길이 아니다.
README 에는 링크를 하나만 둔다. 배치 규칙상 테마마다 링크를 늘릴 수 없다.
