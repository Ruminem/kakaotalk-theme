...
말풍선과 실행화면은 손으로 그리지 않고 스크립트가 만든다.

```
python tools/gen-images.py
```

색을 바꾸려면 그 파일 위쪽 팔레트만 고치고 다시 돌린다. 결과 PNG 는 저장소에 커밋한다 —
빌드 스크립트가 파이썬을 요구하지 않도록.

말풍선은 **세로 그라데이션**이다. 가로로 늘어날 때 각 줄의 색이 유지되기 때문이다.
가로 그라데이션이면 늘어나면서 뭉개진다.
늘어나지 않는 가장자리(`CAP`)는 iOS CSS 의 cap inset 숫자와 같아야 한다.

## 현재 상태

- 탭 아이콘은 양쪽 다 아직 카톡 기본값이다.
- **실행화면(스플래시)은 안드로이드만 된다.** iOS 테마 규격에는 해당 블록이 없다.
