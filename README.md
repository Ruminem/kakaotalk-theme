...
배경과 말풍선을 서로 다른 테마에서 하나씩 골라 섞을 수 있음. 캠핑 밤하늘에 빵집 식빵
말풍선 같은 조합. 깔 것도 로그인도 없음 — 사파리에서 열고 고르면 됨.

| | 어디서 옴 |
|---|---|
| 채팅방·목록·잠금화면 배경 | `배경` 으로 고른 테마 |
| 말풍선 모양, 프로필, 탭 아이콘 | `말풍선` 으로 고른 테마 |
| 말풍선 색 | `말풍선 색` 에서 따로 고름 (네온 계열만) |

1. 위 링크를 **사파리**에서 엶
2. `배경` · `말풍선` 을 고름. 고를 때마다 채팅방 미리보기가 바뀜.
   말풍선을 고를 때는 배경을 뺀 말풍선만 보여줌
3. **카카오톡으로 보내기** → 아무 채팅방에나 보냄 (나와의 채팅도 됨)
4. 보낸 파일을 눌러 **설치**

**말풍선 색은 네온 계열에서 바꿀 수 있음.** 열두 짝이 준비돼 있고 직접 골라도 됨.
색을 바꾸면 프로필·탭 아이콘·잠금화면 점·글자색까지 같이 따라감 — 채팅방만 바뀌고
목록은 옛 색으로 남으면 한 테마로 안 보임. 벽의 낙서는 `배경` 쪽 것이라 그대로임.

> 네온만 되는 이유는 말풍선 그림에 그 색밖에 없어서임. 우표·잼·나사처럼 말풍선 색이
> 아닌 소품이 있는 계열은 색을 돌리면 소품까지 같이 돌아 버림. 어느 계열이 되는지는
> 짐작이 아니라 **재서** 정함 — 색만 돌린 그림과 그 색으로 다시 그린 그림을 대조해
> 차이가 작은 것만 목록에 올림.

미리보기는 실제로 받은 테마 파일 안의 그림과 CSS 로 그림. 말풍선 늘이는 방식도 카톡과 같음.
조합마다 테마 ID 가 달라서 원래 테마를 덮어쓰지 않고 따로 깔림.

> **만든 테마는 채팅방을 한 번 거쳐야 함.** 이미 나와 있는 테마는 카톡 안에서 링크를
> 누르면 바로 깔리는데, 그건 사이트에 진짜 파일이 있어서임. 여기서 만든 건 브라우저
> 안에만 있어서 카톡에 넘겨줄 주소가 없음. 그래서 인앱 브라우저 말고 사파리에서 엶.

> **안드로이드는 이 페이지에서 못 만듦.** `.apk` 는 서명이 있어야 해서 어딘가에서
> 빌드가 돌아야 함. 아래 Actions 쪽을 쓰면 됨.

### 안드로이드도 같이 (Actions)

포크한 저장소의 **Actions** 탭 → **커스텀 테마 제작** → **Run workflow** → 칸 채우고 실행.
3분쯤 뒤 그 런의 **Artifacts** 에 `custom.ktheme` · `custom.apk` · `preview.png` 가 붙음.
여기는 파이썬이 말풍선을 **다시 그림**. 그래서 네온이 아닌 계열도 색을 바꿀 수 있고
**포인트색**(알림 숫자·전송 단추)도 따로 고를 수 있음.

배경과 말풍선에 적는 이름은 **받기 링크에 있는 그 이름**임 — `camp-dark-image`, `bakery-light` 처럼.
카테고리 문서에서 `iOS` 링크 주소 끝을 보면 됨.

> 이쪽은 서명 키를 그때그때 새로 만들어 씀. 같은 조합으로 다시 만들면 안드로이드가
> 다른 앱으로 보기 때문에 **폰에서 먼저 지우고 깔아야 함**. iOS 는 덮어써도 됨.
...
묶여 있어서임. 모양만 떼면 깨짐. 웹 페이지 쪽은 양쪽 다 **배경 그림이 있는 테마**만 고르게
해둠 — 한쪽에 배경이 없으면 CSS 줄을 넣고 빼야 하는데 브라우저에서 할 일이 아님.
배경 없는 테마가 좋으면 그 테마를 그냥 받으면 됨.
