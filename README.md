...
> **안드로이드는 이 페이지에서 못 만듦.** 안드로이드 테마는 **앱**이라 설치하려면 서명이
> 필요한데 브라우저가 그걸 못 함. 아이폰 테마는 그냥 압축 파일이라 됨.
> 안드로이드는 아래 방법으로 만들면 됨.

### 안드로이드 커스텀 테마 제작

**고르는 건 같은 페이지에서 하고, 만드는 것만 깃허브에 맡김.** 깃허브 로그인이 필요하고 3분쯤 걸림.

1. [커스텀 테마 제작](https://ruminem.github.io/kakaotalk-theme/docs/make.html) 을 안드로이드에서 엶
2. `배경` · `말풍선` 을 고름
3. 맨 아래 **깃허브에 만들어 달라고 하기** → 고른 값이 채워진 창이 열림 → **Create** 를 누름
4. 3분쯤 뒤 그 글에 받는 곳이 댓글로 달림. 눌러서 zip 을 받고 풀면 `.apk` 가 있음
...
이쪽은 말풍선을 아예 다시 그리기 때문에 **네온이 아닌 계열도 색을 바꿀 수 있고**,
포인트색(알림 숫자·전송 단추)도 따로 고를 수 있음.

> 만들 때마다 서명이 새로 생김. 같은 조합을 다시 만들면 안드로이드가 다른 앱으로 보니까
> **폰에서 먼저 지우고 깔아야 함**. 아이폰은 덮어써도 됨.
