...
| **탭 아이콘** | 7종 × 보통/선택을 팔레트 색으로 그림. 네온사인과 글로우 변형은 같은 도형의 테두리만 남긴 네온관(선택된 탭만 불이 켜짐), 도트 카테고리는 12칸 도트 | 전부 |
| **기본 프로필 3장** | 색이 다른 세 장. 앱이 친구마다 돌려 배정. 캐릭터 계열은 캐릭터, 네온사인은 하트·별·초승달 네온관, 유리 계열은 배경을 비추는 유리구슬, 나머지는 계열 모티프 세 가지(`tools/motif.py`) | 전부 |
| **목록 셀 투명도** | 목록 배경 그림이 있는 변형은 0.72(유리 0.55, 네온사인 0.5) | 목록 배경 있는 변형 |
