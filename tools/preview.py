...
# 사람 이름. 카톡이 캡처할 때 이름을 가리는 방식("운동하는 라이언")을 따른다.
# 번호만 붙인 더미 이름은 자리를 채운 티가 나서 화면이 덜 진짜 같았다. 모든 테마가 같은
# 이름을 쓴다 — 테마마다 이름이 다르면 나란히 볼 때 색 말고 다른 것이 눈에 걸린다.
NAMES = ('운동하는 라이언', '산책하는 어피치', '낮잠 자는 무지', '춤추는 프로도',
         '쇼핑하는 네오', '요리하는 튜브', '노래하는 제이지', '책 읽는 콘',
         '고구마 먹는 춘식이', '여행하는 죠르디')
...
def chat(t):
    ...
    for i, (side, msg, last, sheet, geo, fw, fh, head, gap) in enumerate(rows):
        ...
        if head:
            ...
            text(d, 54, y + 1, NAMES[0], 12.5, rgb(t['subtext']), anchor='lt')
        fy = y + head
    ...
    text(d, PW / 2, NAV_Y, NAMES[0], 17, rgb(t['text']), 600, 'mm')
...
ROWS = [
    ...
    (NAMES[2], '사진 보냈어 확인해줘', '오후 6:05', 0),
    ('주말 모임', '장소 정했어?', '오후 3:12', 14),
    (NAMES[3], 'ㅋㅋㅋㅋㅋㅋ', '어제', 0),
    (NAMES[4], '고마워!', '어제', 1),
    (NAMES[5], '다음에 또 보자', '9월 10일', 0),
    (NAMES[6], '파일 받았어', '9월 9일', 0),
]
